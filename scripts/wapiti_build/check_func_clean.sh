#!/usr/bin/env bash
# Simulate needed vars/funcs for testing
SVAR_ECHO=""
OUTDIR="/tmp/testout"
has_content() { [[ -f "$1" && -s "$1" ]]; }
log() { echo "$@"; }
...
# Needed for phase_report
st_subs=0; st_live=0; st_ips=0; st_ports=0; st_urls=0; st_js=0; st_params=0
st_nuclei=0; n_critical=0; st_secrets=0; st_cors=0; st_fuzz=0
